...
FINETUNE_LEARNING_RATE = 0.0001 # IMPORTANT: Typically a much smaller LR for fine-tuning the whole backbone
NUM_FINETUNE_EPOCHS = 100 # Can run for more epochs if needed to converge linear head
...
FINETUNE_IMAGE_ROOT_DIR = 'datae/EuroSAT' 
...
class FineTuneModel(nn.Module):
    def __init__(self, pretrained_backbone, num_classes):
        ...
        # The parameters of the backbone will be updated during training because param.requires_grad remains True by default.
        
        self.classifier = nn.Linear(2048, num_classes) 
        
    def forward(self, x):
        # Gradients will now flow through the backbone during backpropagation.
...
def perform_fine_tuning():
    ...
    model = FineTuneModel(pretrained_backbone, NUM_CLASSES)
    model.to(DEVICE)
    print("Fine-tuning model created:")
    print(model)
    
    # --- Data Preparation for EuroSAT (using provided CSV and JSON files) ---
    print("\nInitializing labeled datasets for fine-tuning using CSV splits...")
    ...
    label_map_path = os.path.join(FINETUNE_IMAGE_ROOT_DIR, 'label_map.json')
    train_csv_path = os.path.join(FINETUNE_IMAGE_ROOT_DIR, 'train.csv')
    val_csv_path = os.path.join(FINETUNE_IMAGE_ROOT_DIR, 'validation.csv')
    test_csv_path = os.path.join(FINETUNE_IMAGE_ROOT_DIR, 'test.csv')

    train_dataset = LabeledSatelliteDataset(FINETUNE_IMAGE_ROOT_DIR, train_csv_path, label_map_path, transform=fine_tune_transform)
    val_dataset = LabeledSatelliteDataset(FINETUNE_IMAGE_ROOT_DIR, val_csv_path, label_map_path, transform=fine_tune_transform)
    test_dataset = LabeledSatelliteDataset(FINETUNE_IMAGE_ROOT_DIR, test_csv_path, label_map_path, transform=fine_tune_transform) 
    ...
    val_dataloader = DataLoader(val_dataset, batch_size=FINETUNE_BATCH_SIZE, shuffle=False, 
                                num_workers=os.cpu_count() // 2 if os.cpu_count() > 1 else 0, pin_memory=True)
    test_dataloader = DataLoader(test_dataset, batch_size=FINETUNE_BATCH_SIZE, shuffle=False, 
                                 num_workers=os.cpu_count() // 2 if os.cpu_count() > 1 else 0, pin_memory=True)
    ...
    for epoch in range(NUM_FINETUNE_EPOCHS):
        ...
        pbar_train = tqdm(train_dataloader, desc=f"Fine-tune Train Epoch {epoch+1}/{NUM_FINETUNE_EPOCHS}")
        ...
        val_loss = 0.0
        with torch.no_grad(): 
            pbar_val = tqdm(val_dataloader, desc=f"Fine-tune Val Epoch {epoch+1}/{NUM_FINETUNE_EPOCHS}")
    ...
    torch.save(metrics, 'finetune_metrics.pth') 
    print("Fine-tuning metrics updated with final test accuracy.")
